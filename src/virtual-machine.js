...
class VirtualMachine extends EventEmitter {
    ...
    copySprite (targetId) {
        ...
        return sb3.deserialize(json, this.runtime).then(targets => {
            for (let n = 0; n < targets.length; n++) {
                if (targets[n] !== null) {
                    this.runtime.targets.push(targets[n]);
                    targets[n].updateAllDrawableProperties();
            ...
            var pos = this.runtime.targets.length - 1;
            this.editingTarget = this.runtime.targets[pos];
            this.emitTargetsUpdate();
            this.emitWorkspaceUpdate();
            this.runtime.setEditingTarget(this.editingTarget);
        });
    };
    renameCostumeById (index, newName){
        ...
        if (target) {
            ...
            if (newName && RESERVED_NAMES.indexOf(newName) === -1) {
                const names = costumes
                    .filter(costumeObj => costumeObj.assetId !== costume.assetId)
                    .map(costumeObj => costumeObj.name);
     ...
     * @param {string} newName New name of the sprite.
     */
    renameCostume (costumeId, newName) {
        const target = this.editingTarget;
        if (target) {
            if (!target.isSprite() && !target.isStage) {
                throw new Error('Cannot rename non-sprite targets.');
            ...
            const costumes = sprite.costumes;
            const costume = this.runtime.getCostumeById(sprite, costumeId);
            if (newName && RESERVED_NAMES.indexOf(newName) === -1) {
                const names = costumes
                    .filter(costumeObj => costumeObj.assetId !== costume.assetId)
                    .map(costumeObj => costumeObj.name);
    ...
    renameSound (indexId, newName) {
        ...
        if (target) {
            ...
            const sound = sounds[indexId];
            if (newName && RESERVED_NAMES.indexOf(newName) === -1) {
                const names = sounds
                    .filter(soundObj => soundObj == sound)
                    .map(soundObj => soundObj.name);
